...
let dtr = function (v) { return v * Math.PI / 180; };
//cordinate system transformation.
//polar to rectangle.
let polarToRectangle = function (dX, dY, radius) {
...
//rectangle to polar.
let rectangleToPolar = function (x, y, z) {
  let xD, yD, zD
  if (x == 0)  xD = 0.001;
  else  xD ;
  if (y == 0)  yD = 0.001;
  ...
  let radius = Math.sqrt(xD * xD + yD * yD + zD * zD);
  let theta = Math.atan(zD / Math.sqrt(xD * xD + yD * yD));
  let phi = Math.atan(yD / xD);
...
sphere.prototype = {
  update: function () {
    ...
    for (let i = 0; i < this.charsMap[this.type].length; i++) {
      ...
      if (vibrateFlag == true) {
        let getPosition = polarToRectangle(this.degree[i].theta + 90, this.degree[i].phi, this.radius + Math.random() * 10);
      } else {
        let getPosition = polarToRectangle(this.degree[i].theta + 90, this.degree[i].phi, this.radius);
      };
...
let textChanger = function (text, sphereRadius, sphereSpace, unitTime) {
...
let fullSet = function () {
  let alpha = "ABCDEFGHIJKLMNOPQRSTUVWXYZ__?!1234567890";
  let col = 10;
  let colUnit = 80;
  let rowUnit = 120;
...
let textSet = [
  { text: "TROPIC", sphereRadius: 140, sphereSpace: 80, unitTime: 100, time: 1000 },
  { text: "LAB", sphereRadius: 120, sphereSpace: 70, unitTime: 120, time: 4000 },
  { text: "EXPERIMENTAL", sphereRadius: 120, sphereSpace: 70, unitTime: 50, time: 2000 },
  { text: "SITE", sphereRadius: 120, sphereSpace: 70, unitTime: 100, time: 4000 },
  { text: "MAKE", sphereRadius: 100, sphereSpace: 60, unitTime: 100, time: 3000 },
  { text: "GREAT", sphereRadius: 140, sphereSpace: 80, unitTime: 100, time: 2000 },
  { text: "@@@@@@@@", sphereRadius: 60 + Math.random() * 60, sphereSpace: 200, unitTime: 100, time: 4000 },
  { text: "DO", sphereRadius: 120, sphereSpace: 70, unitTime: 100, time: 2000 },
  { text: "IT", sphereRadius: 100, sphereSpace: 60, unitTime: 150, time: 3500 },
  { text: "@@@@@@@@", sphereRadius: 60 + Math.random() * 60, sphereSpace: 200, unitTime: 100, time: 4000 }
];

let textSetChangerIncrement = 0;
let textSetChanger = function () {
  setTimeout(function () {
    textChanger(
...
let vibrateCV = new closeValue(200, 500);
let invertCV = new closeValue(1000, 1200);
